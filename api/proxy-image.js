...
export default async function handler(req, res) {
  ...
  // ── GET ?charts=fetch-tracks (cron/admin trigger) ──────────────────────────
  // Fetches REAL per-country/global Spotify "Regional Top 200" track chart
  // positions (official charts.spotify.com data) for BLACKPINK + members and
  // stores them to chart_positions. Same auth mechanism as fetch-artists;
  // runs on Vercel because Spotify's anti-bot layer blocks GitHub Actions IPs.
  if (req.query.charts === 'fetch-tracks') {
    const cronSecret = process.env.CRON_SECRET;
    const adminSecret = process.env.ADMIN_SECRET;
    const bearer = req.headers.authorization === `Bearer ${cronSecret}` && cronSecret;
    const adminKey = (req.headers['x-admin-secret'] || req.query.key) === adminSecret && adminSecret;
    ...
    try {
      ...
      for (const chartType of ['daily', 'weekly']) {
        for (let i = 0; i < TRACK_REGIONS.length; i += 15) {
          const batch = TRACK_REGIONS.slice(i, i + 15);
          const results = await Promise.all(batch.map(async region => {
            const alias = `regional-${region}-${chartType}`;
            const r = await fetch(`https://charts-spotify-com-service.spotify.com/auth/v0/charts/${alias}/latest`, {
              headers: { Authorization: `Bearer ${token}`, Accept: 'application/json', 'App-Platform': 'Browser', 'User-Agent': CHARTS_UA },
            });
            if (!r.ok) return { ok: false, region, status: r.status, rows: [] };
            const data = await r.json();
            const entries = data?.displayChart?.entries ?? [];
            const rows = [];
            for (const entry of entries) {
              const trackUri = entry.trackMetadata?.trackUri ?? '';
              const trackId = trackUri.startsWith('spotify:track:') ? trackUri.slice('spotify:track:'.length) : null;
              if (!trackId) continue;
              const artists = entry.trackMetadata?.artists ?? [];
              const matchedArtist = artists.find(a => {
                const aid = (a.spotifyUri || '').startsWith('spotify:artist:') ? a.spotifyUri.slice('spotify:artist:'.length) : null;
                return aid && CHARTS_TRACKED_ARTISTS[aid];
              });
              if (!matchedArtist) continue;
              const matchedArtistId = matchedArtist.spotifyUri.slice('spotify:artist:'.length);
              const d = entry.chartEntryData;
              rows.push({
                spotify_track_id: trackId,
                track_name: entry.trackMetadata?.trackName ?? '',
                primary_artist_id: matchedArtistId,
                primary_artist_name: CHARTS_TRACKED_ARTISTS[matchedArtistId],
                featured_artists: artists
                  .filter(a => {
                    const aid = (a.spotifyUri || '').startsWith('spotify:artist:') ? a.spotifyUri.slice('spotify:artist:'.length) : null;
                    return aid !== matchedArtistId;
                  })
                  .map(a => a.name),
                position: d.currentRank,
                peak_position: d.peakRank ?? null,
                days_on_chart: d.appearancesOnChart ?? null,
                streams: d.relevantMetric ?? d.streams ?? null,
                streams_change: null,
                total_streams: null,
            ...
            return { ok: true, region, rows };
          }));

          for (const result of results) {
            if (!result.ok) { skipped.push({ chartType, region: result.region, status: result.status }); continue; }
      ...
      if (!allRows.length) {
        return res.status(200).json({ ok: true, date: today, upserted: 0, skipped, message: 'No BLACKPINK/member tracks found' });
      }

      const trackIds = [...new Set(allRows.map(r => r.spotify_track_id))];
      const countries = [...new Set(allRows.map(r => r.country))];

      const priorRes = await sbFetch(
      ...
      const upsertRes = await sbFetch('/chart_positions?on_conflict=spotify_track_id,country,tracking_date,chart_type', {
        method: 'POST',
        headers: { Prefer: 'resolution=merge-duplicates' },
        body: JSON.stringify(upsertRows),
      });
