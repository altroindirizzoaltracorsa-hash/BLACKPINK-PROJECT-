...
export default async function handler(req, res) {
  ...
  if (req.query.artist_streams === 'list') {
    const r = await sbFetch(
      ...
      'artist_daily_stats(date,total_streams,daily_delta,followers,monthly_listeners,world_rank,track_count)' +
  ...
  if (req.query.artist_streams === 'detail') {
    ...
    const [artistRes, historyRes, tracksRes] = await Promise.all([
      ...
      sbFetch(`/artist_daily_stats?artist_id=eq.${artistId}&order=date.desc&limit=7&select=date,total_streams,daily_delta,followers,monthly_listeners,world_rank,track_count`, { headers: { Accept: 'application/json' } }),
      sbFetch(
  ...
  if (req.query.artist_streams === 'csv') {
    ...
    const [artistsRes, statsRes] = await Promise.all([
      sbFetch('/tracked_artists?select=spotify_artist_id,name', { headers: { Accept: 'application/json' } }),
      sbFetch(
        '/artist_daily_stats?select=artist_id,date,total_streams,daily_delta,followers,monthly_listeners,world_rank,track_count&order=artist_id.asc,date.asc',
        { headers: { Accept: 'application/json' } },
      ),
    ]);
    ...
    const csv = toCsv(['artist', 'date', 'total_streams', 'daily_delta', 'followers', 'monthly_listeners', 'world_rank', 'track_count'], rows);
    res.status(200);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
